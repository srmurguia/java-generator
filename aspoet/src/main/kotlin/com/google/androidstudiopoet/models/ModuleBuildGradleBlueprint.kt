...
class ModuleBuildGradleBlueprint(
    ...
    private fun createSetOfMandatoryLibraries(): Set<LibraryDependency> {
        ...
        if (enableKotlin) {
            result.add(LibraryDependency("implementation", "org.jetbrains.kotlin:kotlin-stdlib-jre8:${'$'}kotlin_version"))
        }

        if (generateTests) {
            result.add(LibraryDependency("testImplementation", "junit:junit:4.12"))
        }
